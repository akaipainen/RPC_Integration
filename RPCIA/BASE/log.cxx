...
Log* Log::fgInstance = NULL;

Bool_t Log::fgDebugEnabled = kTRUE;
Bool_t Log::fgCoreEnabled = kFALSE;
...
// copy constructor
  ...
  Fatal("Log", "copy constructor not implemented");
}
 ...
 * RpcIA.Log.EnableDebug
 * RpcIA.Log.GlobalLogLevel
 */
//_____________________________________________________________________________
void Log::ReadEnvSettings()
{
...
// and from environment variables
  ...
  else if (gEnv->Defined("RpcIA.Log.EnableDebug"))
  {
    fgDebugEnabled = gEnv->GetValue("RpcIA.Log.EnableDebug", fgDebugEnabled);
    Info(Form("debug %sabled", ((fgDebugEnabled) ? "en" : "dis")));
  }

  // global log level
  if (gEnv->Defined("RpcIA.Log.GlobalLogLevel"))
  {
    const char* type = gEnv->GetValue("RpcIA.Log.GlobalLogLevel", "");
    ...
    Debug(3, Form("global log level set to %d", fGlobalLogLevel));
  }

  // global debug level
  if (gEnv->Defined("RpcIA.Log.GlobalDebugLevel"))
  {
    Int_t level = gEnv->GetValue("RpcIA.Log.GlobalDebugLevel", Int_t(fGlobalLogLevel - kDebugOffset));
    if (level < -kDebugOffset) level = kDebugOffset;
    fGlobalLogLevel = kDebugOffset + level;
    Debug(3, Form("global debug level set to %d", fGlobalLogLevel - kDebugOffset));
  }

  // module debug level
  if (gEnv->Defined("RpcIA.Log.ModuleDebugLevel"))
  {
    TString levels = gEnv->GetValue("RpcIA.Log.ModuleDebugLevel", "");
    ...
    while (const char* module = strtok(p, " "))
    {
      ...
      *(pos++) = '\0';
      ...
      Debug(3, Form("debug level for module %s set to %d", module, level));
    }
  }

  // class debug level
  if (gEnv->Defined("RpcIA.Log.ClassDebugLevel"))
  {
    TString levels = gEnv->GetValue("RpcIA.Log.ClassDebugLevel", "");
    char* p = const_cast<char*>(levels.Data());
    ...
    while (const char* className = strtok(p, " "))
    {
      ...
      *(pos++) = '\0';
      ...
      Debug(3, Form("debug level for class %s set to %d", className, level));
    }
  }

  // general output stream
  if (gEnv->Defined("RpcIA.Log.Output"))
  {
    TString stream = gEnv->GetValue("RpcIA.Log.Output", "Standard");

    if (stream.CompareTo("standard", TString::kIgnoreCase) == 0)
    {
      SetStandardOutput();
      Debug(3, "output stream set to standard output for all types");
    }
    else if (stream.CompareTo("error", TString::kIgnoreCase) == 0)
    {
      SetErrorOutput();
      Debug(3, "output stream set to error output for all types");
    }
    else if (!stream.IsNull())
    {
      SetFileOutput(stream);
      Debug(3, Form("output stream set to file %s for all types", stream.Data()));
    }
  }
  ...
  for (Int_t iType = kFatal; iType < kMaxType; iType++)
  {
    ...
    if (gEnv->Defined(name))
    {
      ...
      if (stream.CompareTo("standard", TString::kIgnoreCase) == 0)
      {
        ...
        Debug(3, Form("output stream set to standard output for type %s", typeNames[iType]));
      }
      else if (stream.CompareTo("error", TString::kIgnoreCase) == 0)
      {
        SetErrorOutput(EType_t(iType));
        Debug(3, Form("output stream set to error output for type %s", typeNames[iType]));
      }
      else if (!stream.IsNull())
      {
        SetFileOutput(EType_t(iType), stream);
        Debug(3, Form("output stream set to file %s for type %s", stream.Data(), typeNames[iType]));
      }
    }
  }

  // handling of root error messages
  if (gEnv->Defined("RpcIA.Log.HandleRootMessages"))
  {
    Bool_t on = gEnv->GetValue("RpcIA.Log.HandleRootMessages", kTRUE);
    SetHandleRootMessages(on);
    Debug(3, Form("handling of root messages %sabled", ((on) ? "en" : "dis")));
  }
  ...
  for (Int_t iSetting = 0; iSetting < 4; iSetting++)
  {
    ...
    if (gEnv->Defined(name))
    {
      ...
      Debug(3, Form("printing of %s %sabled for all types", settingNames[iSetting], ((on) ? "en" : "dis")));
    }
    ...
    for (Int_t iType = kFatal; iType < kMaxType; iType++)
    {
      ...
      if (gEnv->Defined(nameType))
      {
        ...
        settings[iSetting][iType] = on;
        Debug(3, Form("printing of %s %sabled for type %s", settingNames[iSetting], ((on) ? "en" : "dis"), typeNames[iType]));
      }
    }
  }

  // repetition of messages
  if (gEnv->Defined("RpcIA.Log.PrintRepetitions"))
  {
    Bool_t on = gEnv->GetValue("RpcIA.Log.PrintRepetitions", kTRUE);
    fPrintRepetitions = on;
    Debug(3, Form("printing of message repetitions %sabled", ((on) ? "en" : "dis")));
  }
  if (gSystem->Getenv("RPCIA_FORCE_COREDUMP")){
...
//_____________________________________________________________________________
void Log::RootErrorHandler(Int_t level, Bool_t abort, 
...
// new error handler for messages from root
  ...
  Log::Message(level, message, "ROOT", NULL, location, NULL, 0);
}


// DEPRECATED: USE A CONFIGURATION FILE INSTEAD
//_____________________________________________________________________________
void Log::EnableDebug(Bool_t enabled)
{
...
void Log::EnableCoreDump(Bool_t enabled)
{
// enable or disable debug output
...
//_____________________________________________________________________________
void Log::SetGlobalLogLevel(EType_t type)
{
// set the global debug level

  // TO BE DELETED
  if (!fgInstance) new Log; 
  fgInstance->fGlobalLogLevel = type;
}

//_____________________________________________________________________________
Int_t Log::GetGlobalLogLevel()
{
...
//_____________________________________________________________________________
void Log::SetGlobalDebugLevel(Int_t level)
{
// set the global debug level
...
//_____________________________________________________________________________
void Log::SetModuleDebugLevel(const char* module, Int_t level)
{
...
//_____________________________________________________________________________
void Log::ClearModuleDebugLevel(const char* module)
{
...
//_____________________________________________________________________________
void Log::SetClassDebugLevel(const char* className, Int_t level)
{
...
//_____________________________________________________________________________
void Log::ClearClassDebugLevel(const char* className)
{
...
//_____________________________________________________________________________
void Log::SetStandardOutput(EType_t type)
{
...
//_____________________________________________________________________________
void Log::SetErrorOutput(EType_t type)
{
...
//_____________________________________________________________________________
void Log::SetFileOutput(const char* fileName)
{
...
//_____________________________________________________________________________
void Log::SetFileOutput(EType_t type, const char* fileName)
{
...
//_____________________________________________________________________________
FILE* Log::GetOutputStream(Int_t type)
{
...
//_____________________________________________________________________________
void Log::SetHandleRootMessages(Bool_t on)
{
...
//_____________________________________________________________________________
void Log::SetPrintType(Bool_t on)
{
...
//_____________________________________________________________________________
void Log::SetPrintType(EType_t type, Bool_t on)
{
...
//_____________________________________________________________________________
void Log::SetPrintModule(Bool_t on)
{
...
//_____________________________________________________________________________
void Log::SetPrintModule(EType_t type, Bool_t on)
{
...
//_____________________________________________________________________________
void Log::SetPrintScope(Bool_t on)
{
...
//_____________________________________________________________________________
void Log::SetPrintScope(EType_t type, Bool_t on)
{
...
//_____________________________________________________________________________
void Log::SetPrintLocation(Bool_t on)
{
...
//_____________________________________________________________________________
void Log::SetPrintLocation(EType_t type, Bool_t on)
{
// switch on or off the printing of the file name and line number 
// for the given message type
...
//_____________________________________________________________________________
void Log::SetPrintRepetitions(Bool_t on)
{
...
//_____________________________________________________________________________
void Log::WriteToFile(const char* name, Int_t option)
{
...
//_____________________________________________________________________________
UInt_t Log::GetLogLevel(const char* module, const char* className) const
{
...
//_____________________________________________________________________________
Int_t Log::GetDebugLevel(const char* module, const char* className)
{
...
//_____________________________________________________________________________
void Log::PrintMessage(UInt_t type, const char* message, 
...
//_____________________________________________________________________________
void Log::Message(UInt_t level, const char* message, 
		     const char* module, const char* className,
		     const char* function, const char* file, Int_t line)
{
// print a log message
  ...
  if (type == kFatal) {
    fgInstance->PrintMessage(type, "aborting execution due to Fatal", 
                             module, className, function, file, line);
    delete fgInstance;
    if (gSystem) {
      gSystem->StackTrace();
      if (fgCoreEnabled) MakeCoreDump("core.RpcIA");
      gSystem->Abort();
    } else {
      if (fgCoreEnabled) MakeCoreDump("core.RpcIA");
      ::abort();
...
//_____________________________________________________________________________
void Log::Debug(UInt_t level, const char* message, 
		   const char* module, const char* className,
		   const char* function, const char* file, Int_t line)
{
...
//_____________________________________________________________________________
Int_t Log::RedirectStdoutTo(EType_t type, UInt_t level, const char* module, 
...
//_____________________________________________________________________________
Int_t Log::RedirectStderrTo(EType_t type, UInt_t level, const char* module, 
                               const char* className, const char* function,
                               const char* file, Int_t line, Bool_t print)
{
...
//_____________________________________________________________________________
Int_t Log::RedirectTo(FILE* stream, EType_t type, UInt_t level, 
                         const char* module, const char* className,
                         const char* function, const char* file, Int_t line,
			 Bool_t print)
{
// redirect the standard (error) output stream to the stream of the given type
  ...
  if (type + level > GetLogLevel(module, className)) { // /dev/null
    if(!freopen("/dev/null", "a", stream)) Warning("Cannot reopen /dev/null");
    ...
    if(!freopen(fFileNames[type], "a", stream)) Warning(Form("Cannot reopen %s",fFileNames[type].Data()));
...
//_____________________________________________________________________________
void Log::RestoreStdout(Int_t original)
{
...
//_____________________________________________________________________________
void Log::RestoreStderr(Int_t original)
{
...
//_____________________________________________________________________________
ostream& Log::Stream(EType_t type, UInt_t level,
                        const char* module, const char* className,
                        const char* function, const char* file, Int_t line)
{
...
//_____________________________________________________________________________
ostream& Log::GetStream(EType_t type, UInt_t level,
                           const char* module, const char* className,
                           const char* function, const char* file, Int_t line)
{
// get the stream object for the given output type
...
void  Log::SetStreamOutput(ostream* stream)
{
  ...
  // Log class just writes to it

  for (Int_t iType = kFatal; iType < kMaxType; iType++) {
    SetStreamOutput((Log::EType_t)iType, stream);
  }
}

void  Log::SetStreamOutput(EType_t type, ostream* stream)
{
...
void  Log::SetLogNotification(LogNotification pCallBack)
{
  // set a notification callback function for log messages of all types

  for (Int_t iType = kFatal; iType < kMaxType; iType++) {
    SetLogNotification((Log::EType_t)iType, pCallBack);
  }
}

void  Log::SetLogNotification(EType_t type, LogNotification pCallBack)
{
...
void  Log::PrintString(Int_t type, FILE* stream, const char* format, ...)
{
...
void Log::MakeCoreDump(const char *fout){
  //
  ...
  //
  // Example use - make default core file for current process:  Log::MakeCoreDump(0)
  //
  //
  // Automatic core dump creation in case of the Fatal can be specified using
  // static void  EnableCoreDump(Bool_t enabled);
  // Core dump is created in addition to the stack trace ()  
  // marian.ivanov@cern.ch
  //
  if (!gSystem) return;
  printf("Log::MakeCoreDump\n");
  if (fout){
...
void Log::TestException(Int_t level){
  //
  // Dummy function to throw exception
  //
  printf("Log::TestException(%d)\n",level);
  if (level>0){
    level--;
    TestException(level);
  }else{
